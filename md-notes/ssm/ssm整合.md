...
- [ssm整合](#ssm整合)
    - [1.ssm整合](#1ssm整合)
        - [1-1.搭建整合环境](#1-1搭建整合环境)
            - [1-1-1.使用XML+注解的方式进行ssm的整合](#1-1-1使用xml注解的方式进行ssm的整合)
            - [1-1-2.整合思路](#1-1-2整合思路)
            - [1-1-3.创建数据库和表](#1-1-3创建数据库和表)
        - [1-2.整合前准备](#1-2整合前准备)
            - [1-2-1.创建maven的web工程](#1-2-1创建maven的web工程)
            - [1-2-2.添加整合所需要的maven依赖包](#1-2-2添加整合所需要的maven依赖包)
            - [1-2-3.创建实体类](#1-2-3创建实体类)
            - [1-2-4.创建dao](#1-2-4创建dao)
            - [1-2-5.创建service](#1-2-5创建service)
            - [1-2-6.创建Controller](#1-2-6创建controller)
        - [1-3.搭建Spring环境](#1-3搭建spring环境)
            - [1-3-1.applicationContext.xml](#1-3-1applicationcontextxml)
            - [1-3-2.log4j.properties日志文件](#1-3-2log4jproperties日志文件)
            - [1-3-3.修改service，加上@Service注解](#1-3-3修改service加上service注解)
            - [1-3-4.测试Spring的环境](#1-3-4测试spring的环境)
        - [1-4.搭建SpringMVC的环境](#1-4搭建springmvc的环境)
            - [1-4-1.web.xml和springmvc.xml配置文件](#1-4-1webxml和springmvcxml配置文件)
            - [1-4-2.AccountController.java](#1-4-2accountcontrollerjava)
            - [1-4-3.index.jsp](#1-4-3indexjsp)
            - [1-4-4.success.jsp(/WEB-INF/pages/目录下)](#1-4-4successjspweb-infpages目录下)
        - [1-5.Spring整合SpringMVC框架](#1-5spring整合springmvc框架)
            - [1-5-1.web.xml](#1-5-1webxml)
            - [1-5-2.AccountController.java](#1-5-2accountcontrollerjava)
        - [1-6.搭建Mybatis的环境](#1-6搭建mybatis的环境)
            - [1-6-1.sqlMapConfig.xml](#1-6-1sqlmapconfigxml)
            - [1-6-2.AccountDao.java](#1-6-2accountdaojava)
            - [1-6-3.MybatisTest.java](#1-6-3mybatistestjava)
        - [1-7.Spring整合Mybatis框架](#1-7spring整合mybatis框架)
            - [1-7-1.applicationContext.xml](#1-7-1applicationcontextxml)
            - [1-7-2.AccountDao.java](#1-7-2accountdaojava)
            - [1-7-3.AccountServiceImpl.java](#1-7-3accountserviceimpljava)
            - [1-7-4.AccountController.java](#1-7-4accountcontrollerjava)
            - [1-7-5.success.jsp(/WEB-INF/pages/目录下)](#1-7-5successjspweb-infpages目录下)
            - [1-7-6.测试](#1-7-6测试)
        - [1-8.配置Spring的声明式事务管理](#1-8配置spring的声明式事务管理)
            - [1-8-1.applicationContext.xml](#1-8-1applicationcontextxml)
            - [1-8-2.index.jsp](#1-8-2indexjsp)
            - [1-8-3.AccountController.java](#1-8-3accountcontrollerjava)
            - [1-8-4.测试](#1-8-4测试)
        - [1-9.让SpringMVC扫描，不让Spring扫描](#1-9让springmvc扫描不让spring扫描)
            - [1-9-1.配置springmvc.xml](#1-9-1配置springmvcxml)
            - [1-9-2.配置springContext.xml](#1-9-2配置springcontextxml)
            - [1-9-3.去掉web.xml中的监听器](#1-9-3去掉webxml中的监听器)
    - [2.SSM整合案例-用户注册/登录](#2ssm整合案例-用户注册登录)
        - [2-1.ssm配置](#2-1ssm配置)
            - [2-1-1.web.xml](#2-1-1webxml)
            - [2-1-2.applicationContext.xml](#2-1-2applicationcontextxml)
            - [2-1-3.springmvc.xml](#2-1-3springmvcxml)
        - [2-2.用户注册](#2-2用户注册)
            - [2-2-1.数据库表设计](#2-2-1数据库表设计)
            - [2-2-2.创建工程并添加maven依赖](#2-2-2创建工程并添加maven依赖)
            - [2-2-3.创建User.java](#2-2-3创建userjava)
            - [2-2-4.注册页面](#2-2-4注册页面)
            - [2-2-5.创建UserController.java](#2-2-5创建usercontrollerjava)
            - [2-2-6.创建接口UserService和实现类UserServiceImpl](#2-2-6创建接口userservice和实现类userserviceimpl)
            - [2-2-7.创建接口UserDao](#2-2-7创建接口userdao)
            - [2-2-8.register.html测试](#2-2-8registerhtml测试)
            - [2-2-9.关于路径问题](#2-2-9关于路径问题)
        - [2-3.用户注册](#2-3用户注册)
            - [2-3-1.login.html登录页面](#2-3-1loginhtml登录页面)
            - [2-3-2.在WEB-INF/pages/目录下新建welcome.jsp](#2-3-2在web-infpages目录下新建welcomejsp)
            - [2-3-3.UserController.java](#2-3-3usercontrollerjava)
            - [2-3-4.UserServiceImpl.java](#2-3-4userserviceimpljava)
            - [2-3-5.UserDao.java](#2-3-5userdaojava)
            - [2-3-6.登录成功，重定向到欢迎页](#2-3-6登录成功重定向到欢迎页)
        - [2-4.整合拦截器](#2-4整合拦截器)
            - [2-4-1.LoginInterceptor.java](#2-4-1logininterceptorjava)
            - [2-4-2.springmvc.xml](#2-4-2springmvcxml)
    - [3.Maven的tomcat插件使用](#3maven的tomcat插件使用)
        - [3-1.在pom.xml中配置tomcat插件](#3-1在pomxml中配置tomcat插件)
        - [3-2.去掉web项目的tomcat配置](#3-2去掉web项目的tomcat配置)
        - [3-3.使用maven来跑tomcat](#3-3使用maven来跑tomcat)
        - [3-4.测试](#3-4测试)
